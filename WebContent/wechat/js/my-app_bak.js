		 ...
		 $("#brands").parents(".cached").remove();//删除缓存
			...
			function checkOk(){
				...
				if(!$("#username").val()){
					...
					isOk=false;
				}else if(!$("#cameraInput").get(0).files[0]){
					isOk=false;
				}else{
			...
			$("#submitOrder").on("click",function(){
				...
				formdata.append("phone",$("#phone").val());//添加电话
				formdata.append("storeId",$("#stores").val());//添加店铺
				formdata.append("fare",$("#fare").val());//消费金额
			...
			//	 var queryparam="?bankName="+bankName+"&bankAddress="+bankAddress+"&cardNum="+cardNum+"&cardUsername="+cardUsername
				...
				var mySwiper3 = myApp.swiper('.swiper-between', {
