									...
									<tr>
										...
										<td>
											<a class="btn btn-danger  btn-mini changeStatu" orderid="${temp.orderId  }" status="${temp.status}" info="${temp.info }" href="#"> 
												<i class="halflings-icon edit white"></i> 审核</a> 
										</td>
									</tr>
								</c:forEach>
								</tbody>
...
//	$(".select").chosen();
	...
	//	    shadeClose:true,
		    ...
		    content: '<p class="stLine">订单状态：</p><input type="hidden" id="statusTemp"/><div class="status"><a class="btn  status btn-mini " status="${nextstatus}" href="javascript:;">'+ 
				' 审核通过</a><a class="btn  status btn-mini" status="4" href="javascript:;">'+ 
	...
	$("body").on("click","a.status-submit",function(){
		...
		}else if(newstatus=='4'||newstatus=='2'){
