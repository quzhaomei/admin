...
<body>
							...
							<table class="table table-striped table-bordered">
								...
								<c:forEach items="${pageResult.param }" var="temp">
									<tr>
										...
										<c:if test="${temp.getMorePay!=null }">
										<span class="label">￥<fmt:formatNumber value="${temp.getMorePay }" maxFractionDigits="2" /> </span>
										</c:if>
										<c:if test="${temp.getMorePay==null }">
										<span class="label">￥<fmt:formatNumber value="${temp.hasPay*temp.brand.rate }" maxFractionDigits="2" /> </span>
										</c:if>
										
										</td>
										<td>${temp.backType=='0'?"微信支付":"<span class='label label-error'>网银支付</span>" }</td>
										<td>
										...
										<td>
										<c:if test="${temp.backType=='1' }"><!-- 银行返利显示卡号 -->
										<a class="btn btn-success  btn-mini checkCard" getMoreId="${temp.getMoreId }" href="#"> 
												<i class="halflings-icon edit white"></i> 银行卡号</a>
										</c:if>
	...
	<div class="modal hide fade" id="bankCard">
	<div class="modal-header">
		...
		<table class="table">
		<tr>
		<td>银行名称：</td><td><span id="bankName"></span></td>
		</tr>
		<tr>
		<td>开户行：</td><td><span id="accountAddr"></span></td>
		</tr>
		<tr>
		<td>银行卡号：</td><td><span id="cardNo"></span></td>
		</tr>
		<tr>
		<td>名字：</td><td><span id="username"></span></td>
		</tr>
		</table>
		</div>
...
//	$(".select").chosen();
	...
	$(".changeStatu").on("click",function(){
		...
		layer.open({
		    ...
		    area: ['520px', '340px'], //宽高
		    content: '<p class="stLine">订单状态：</p><input type="hidden" id="statusTemp"/><div class="status"><a class="btn  status btn-mini " status="${nextstatus}" href="javascript:;">'+ 
				' 确定支付</a><a class="btn  status btn-mini" status="4" href="javascript:;">'+ 
