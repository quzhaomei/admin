 ...
 */
@Controller
@RequestMapping(value = "/yuandan")
@LimitTag
public class PhoneRegisterYuandanController extends BaseController {
	private static final Integer TYPE = 1;// 元旦活动
	...
	public String index(HttpServletRequest request,
		...
		if (startTimeStr != null
				&& startTimeStr.matches("\\d{4}-\\d{2}-\\d{2}")) {
			try {
		...
		PageDTO<List<PhoneRegister>> pageData = registerService
				.getPageByParamAndTime(pageParam, startTime, endTime);
		...
		model.addAttribute("activeName", "元旦活动");
		return "minisite/phoneRegisterYuandan";
	}
	...
	public String list(HttpServletRequest request,
		...
		try {
			...
			WritableSheet ws = wwb.createSheet("元旦活动注册名单", 0);// 建立工作簿
			// 写表头
			...
			jxl.write.Label label4 = new jxl.write.Label(3, 0, "发起时间");
			...
			for (int i = 0; i < result.size(); i++) {
				...
				jxl.write.Label temp1 = new jxl.write.Label(0, i + 1, ""
						+ (i + 1));
				jxl.write.Label temp2 = new jxl.write.Label(1, i + 1,
						temp.getName());
				jxl.write.Label temp3 = new jxl.write.Label(2, i + 1,
						temp.getPhone());
				jxl.write.Label temp4 = new jxl.write.Label(3, i + 1,
						dateFormat.format(temp.getCreateDate()));
	...
	public String saveOrUpdate(HttpServletRequest request,
		...
		if ("add".equals(operator)) {
			if (name != null && phone != null) {
				...
				if (readyTime != null
						&& readyTime.matches("\\d{4}-\\d{2}-\\d{2}")) {// 设置预购时间
					try {
			...
			if (phone != null && content != null) {
				try {
					...
					// 为null，发送测试数据
					HttpSendResult result = HttpSender.sendMsg(phone, content);
					model.addAttribute("json", JSONUtil.object2json(result));
				} catch (Exception e) {
					model.addAttribute("json", JSONUtil
							.object2json(new HttpSendResult(false, "系统出现异常")));
					e.printStackTrace();
				}
			}
			return "json";
			...
			if (phone != null) {
				...
				// 查看手机号是否重复
				PhoneRegister oldParam = registerService
						.getByParam(updateParam);
